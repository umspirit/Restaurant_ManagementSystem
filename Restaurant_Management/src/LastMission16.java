...
public class LastMission16 extends JFrame {
		...
		class ClickAction implements ActionListener {
			public void actionPerformed(ActionEvent e) {
				if (e.getSource() == Clickbtn1) {
					...
					setModal(false);
					setVisible(false);
					int result = JOptionPane.showConfirmDialog(null, "예약하시겠습니까 ?",
							"Confirm", JOptionPane.YES_NO_OPTION);
					if(result == JOptionPane.YES_OPTION){
						setVisible(false);
						namefield.setText("");
	...
	class Reservation extends JDialog {
		...
		class MyActionListener implements ActionListener {
			public void actionPerformed(ActionEvent e) {
				...
				if (e.getSource() == mn1) {   				   // 미션 16 JDlalog 에서 텍스트파일 읽기
					Stringnum = 0;
					try {
				...
				} else if (e.getSource() == mn2) { 				   // 미션 16 JDlalog 에서 텍스트파일 읽기
					Stringnum = 0;
					try {
					...
					hpfield3.setText(Stringtxt[4]);
					man.setSelected(true);
				} else if (e.getSource() == mn3) { 				   // 미션 16 JDlalog 에서 텍스트파일 읽기
					Stringnum = 0;
					try {
					...
					hpfield2.setText(Stringtxt[3]);
					hpfield3.setText(Stringtxt[4]);
				} else if (e.getSource() == mn4) { 				   // 미션 16 JDlalog 에서 텍스트파일 읽기
					Stringnum = 0;
					try {
